"""
DDPM Face Generation — Precision Tuning (v0.7)
===============================================
Goal: Push FID below v0.6's 66.20 through four well-motivated refinements,
      each rooted in recent diffusion-model literature.

Target: FID ~55 (≈15-20% improvement over v0.6 Phase 3).
Beyond this, significant further improvement likely requires latent diffusion
or substantially more training data (~100k+ images).

Four documented improvement phases (each builds on the previous):
  Phase 1 – Zero Terminal SNR rescaling
  Phase 2 – + Input perturbation (γ = 0.1)
  Phase 3 – + Stronger data augmentation (color jitter + RandomResizedCrop)
  Phase 4 – + Extended training (120 epochs)

Changes vs v0.6.1:
  [SCHED] Zero Terminal SNR rescaling (Lin et al. 2024)
          Paper: "Common Diffusion Noise Schedules and Sample Steps are Flawed"
          Even with shifted cosine, ᾱ(T) ≈ 0.003 not 0 → model never actually
          sees pure noise during training, but sampling starts from pure noise.
          Fix: rescale sqrt(ᾱ) so terminal value is exactly 0. Formula:
              s = sqrt(ᾱ);  s ← (s - s[-1]) · s[0] / (s[0] - s[-1])
          Closes train/test gap. Empirically gives 10-20% FID improvement
          across datasets.

  [TRAIN] Input perturbation with γ = 0.1 (Ning et al. 2023)
          Paper: "Input Perturbation Reduces Exposure Bias in Diffusion Models"
          During training, sample x₀_pert = x₀ + γ · sqrt(1-ᾱ_t) · N(0, I),
          then run forward process on x₀_pert but keep loss target based on
          original x₀. Acts as targeted regularization in the direction of the
          noise the model will see at inference. γ = 0.1 is the paper's default.

  [DATA]  Stronger augmentation (Phase 3+)
          Adds ColorJitter (brightness/contrast/saturation = 0.1) and
          RandomResizedCrop (scale 0.9–1.0). With only ~30k training images,
          the model benefits substantially from more diverse views.
          FID reference still uses deterministic (non-augmented) view.

  [TRAIN] Extended training to 120 epochs (Phase 4)
          Loss curves in v0.6 showed val_loss still slowly decreasing at
          epoch 80. 50% more training → better convergence, sharper features.

Kept from v0.6.1:
  - 128×128 resolution, 4-stage U-Net [64, 128, 256, 512] (34M params)
  - Self-attention at 16×16 AND 32×32
  - Shifted-cosine schedule (shift = 2.0, Hoogeboom et al. 2023)
  - v-prediction (Salimans & Ho 2022)
  - Min-SNR loss weighting with γ = 5 (Hang et al. 2024)
  - EMA warmup 2000 steps, decay 0.9995
  - batch 32 × grad_accum 2 = effective 64
  - DDIM 50-step sampler for final FID (19× speedup over DDPM)

Usage on HPC:
  sbatch -p normal --qos gpu_batch --gres=gpu:1 --time=16:00:00 \
    --job-name "v07_p1" \
    --wrap "python3 ~/ddpm_pipeline_v0.7.py --phase 1 --log_dir /data/01/up202512956/ddpm_runs_v07"
"""
...
class Config:
    ...
    num_workers:  int       = 4
    # [v0.7] "weak" (flip only) | "strong" (flip + color + resized crop)
    augmentation_level: str = "weak"

    # ── model ─────────────────────────────────────────────────
    channels:     List[int] = field(default_factory=lambda: [64, 128, 256, 512])
    use_attention: bool     = True          # [v0.7] always on from Phase 1
    ...
    # ── noise schedule ────────────────────────────────────────
    T:            int       = 1000
    schedule:     str       = "shifted_cosine"   # [v0.7] default from Phase 1
    beta_start:   float     = 1e-4
    beta_end:     float     = 2e-2
    cosine_shift: float     = 2.0
    # [v0.7 NEW] Zero Terminal SNR rescaling (Lin et al. 2024)
    use_zero_snr: bool      = True

    # ── loss ──────────────────────────────────────────────────
    prediction_type: str    = "v"           # [v0.7] always v from Phase 1
    loss_weighting: str     = "min_snr"     # [v0.7] always min_snr from Phase 1
    min_snr_gamma:  float   = 5.0

    # [v0.7 NEW] Input perturbation scale (Ning et al. 2023).
    # 0.0 disables. Paper recommends 0.1.
    input_pert_gamma: float = 0.0
    ...
    # ── sampling ──────────────────────────────────────────────
    sampler:      str  = "ddim"             # [v0.7] default fast sampler
    sample_steps: int  = 50
    n_samples:    int  = 16

    # ── logging ───────────────────────────────────────────────
    log_dir:          str  = "./ddpm_runs_v07"
    sample_every:     int  = 10
    fid_every:        int  = 20
    checkpoint_every: int  = 20
    phase_name:       str  = "phase1_zsnr"
...
def get_phase_config(phase: int, **overrides) -> Config:
    """
    v0.7 phase structure:
      Phase 1: Zero Terminal SNR baseline
      Phase 2: + Input perturbation (γ=0.1)
      Phase 3: + Stronger augmentation
      Phase 4: + Extended training (120 epochs)
    """
    cfg = Config()
    # Phase 1 is the new baseline: v0.6 Phase 3 + ZSNR
    cfg.phase_name = "phase1_zsnr"
    if phase >= 2:
        cfg.input_pert_gamma = 0.1
        cfg.phase_name = "phase2_input_pert"
    if phase >= 3:
        cfg.augmentation_level = "strong"
        cfg.phase_name = "phase3_strong_aug"
    if phase >= 4:
        cfg.epochs = 120
        cfg.phase_name = "phase4_extended"
...
# ──────────────────────────────────────────────────────────────
# 2.  DATASET  [v0.7: + optional strong augmentation]
# ──────────────────────────────────────────────────────────────

class FaceDataset(Dataset):
    def __init__(self, data_dirs: List[str], image_size: int = 128,
                 augment: bool = True, aug_level: str = "weak"):
        ...
        print(f"Dataset: {len(self.paths):,} images from {len(data_dirs)} dir(s)  "
              f"[augmentation: {'none' if not augment else aug_level}]")
    ...
    def _build_transform(self, augment: bool) -> transforms.Compose:
        ...
        if augment:
            ...
            if self.aug_level == "strong":
                # [v0.7] Stronger augmentation for Phase 3+
                tf_list.append(transforms.ColorJitter(
                    brightness=0.1, contrast=0.1, saturation=0.1
                ))
                tf_list.append(transforms.RandomResizedCrop(
                    self.image_size, scale=(0.9, 1.0), ratio=(0.95, 1.05)
                ))
            else:
                tf_list.append(transforms.Resize((self.image_size, self.image_size)))
        else:
            tf_list.append(transforms.Resize((self.image_size, self.image_size)))
        tf_list += [
            transforms.ToTensor(),
...
def build_dataloaders(cfg: Config):
    full = FaceDataset(cfg.data_dirs, cfg.image_size, augment=True,
                       aug_level=cfg.augmentation_level)
    ...
    # Validation uses same images but WITHOUT augmentation (deterministic FID)
...
# ──────────────────────────────────────────────────────────────
# 3.  NOISE SCHEDULER  [v0.7: + Zero Terminal SNR]
# ──────────────────────────────────────────────────────────────

def enforce_zero_terminal_snr(alphas_cumprod: torch.Tensor) -> torch.Tensor:
    """
    Lin et al. 2024, Algorithm 2.

    Rescales alphas_cumprod so that the terminal value becomes exactly 0,
    i.e. sqrt(ᾱ(T)) = 0 → pure noise at t = T.

    Math:
        s = sqrt(ᾱ)
        s ← s - s[-1]                  (shift end to 0)
        s ← s * s[0] / (s[0] - s[-1])  (preserve starting value)
        return s²
    """
    sqrt_abar = alphas_cumprod.sqrt().clone()

    s_0 = sqrt_abar[0].clone()
    s_T = sqrt_abar[-1].clone()

    # Shift + rescale
    sqrt_abar = sqrt_abar - s_T
    sqrt_abar = sqrt_abar * s_0 / (s_0 - s_T)

    # Numerical safety: clamp to [0, 1]
    sqrt_abar = sqrt_abar.clamp(min=0.0, max=1.0)
...
class NoiseScheduler:
    """
    [v0.7] Supports optional Zero Terminal SNR rescaling on top of any schedule.
    """

    def __init__(self, T=1000, schedule="shifted_cosine", beta_start=1e-4,
                 beta_end=2e-2, cosine_shift=2.0, use_zero_snr=False):
        self.T = T
        self.schedule_name = schedule
        self.use_zero_snr = use_zero_snr

        # Compute base alphas_cumprod exactly as v0.6
        if schedule == "linear":
        ...
        # [v0.7 NEW] Apply zero-terminal-SNR rescaling if requested
        if use_zero_snr:
            alphas_cumprod = enforce_zero_terminal_snr(alphas_cumprod)

        # Derive everything else from alphas_cumprod (same as v0.6)
        # Important: with zero_snr, alphas_cumprod[-1] = 0, so beta_T = 1.
        alphas_cumprod_prev = F.pad(alphas_cumprod[:-1], (1, 0), value=1.0)
        alphas = alphas_cumprod / alphas_cumprod_prev.clamp(min=1e-20)
        alphas = alphas.clamp(min=1e-20, max=1.0)
        betas = (1.0 - alphas).clamp(min=0.0, max=1.0)
        ...
        self.sqrt_one_minus_alphas_cumprod = (1 - alphas_cumprod).clamp(min=0).sqrt()

        # SNR — with zero_snr, SNR(T) = 0, so Min-SNR weight goes to 0 at t=T (fine).
        self.snr = alphas_cumprod / (1 - alphas_cumprod).clamp(min=1e-20)

        # Posterior variance — careful at t=T where alphas_cumprod=0
        self.posterior_variance  = betas * (1 - alphas_cumprod_prev) / (1 - alphas_cumprod).clamp(min=1e-20)
        self.posterior_log_var   = torch.log(self.posterior_variance.clamp(min=1e-20))
        self.posterior_mean_c1   = betas * alphas_cumprod_prev.sqrt() / (1 - alphas_cumprod).clamp(min=1e-20)
        self.posterior_mean_c2   = (1 - alphas_cumprod_prev) * alphas.sqrt() / (1 - alphas_cumprod).clamp(min=1e-20)
    ...
    def predict_x0(self, xt, t, eps):
        s  = self.sqrt_alphas_cumprod[t][:, None, None, None].clamp(min=1e-6)
        s1 = self.sqrt_one_minus_alphas_cumprod[t][:, None, None, None]
        return (xt - s1 * eps) / s
...
def compute_loss_weights(snr: torch.Tensor, t: torch.Tensor,
    ...
    if weighting == "uniform":
        ...
        if prediction_type == "v":
            ...
            # [v0.7] clamp SNR denominator for numerical safety (important with ZSNR)
            return min_snr / snr_t.clamp(min=1e-8)
    else:
        raise ValueError(f"Unknown loss_weighting: {weighting!r}")


# ──────────────────────────────────────────────────────────────
# 5.  U-NET  (unchanged from v0.6)
# ──────────────────────────────────────────────────────────────
...
class UNet(nn.Module):
    """4-stage U-Net for 128×128. Same architecture as v0.5/v0.6."""
...
# ──────────────────────────────────────────────────────────────
# 8.  SAMPLERS  (DDPM + DDIM; v-prediction aware)
# ──────────────────────────────────────────────────────────────
...
def ddim_sample(model, sched, shape, device, n_steps=50, eta=0.0, generator=None, prediction_type="eps"):
    ...
    for i, t_idx in enumerate(tqdm(step_ids, desc="DDIM", leave=False)):
        ...
        eps    = _model_to_eps(model, x, t, sched, prediction_type)
        abar_t = sched.alphas_cumprod[t_idx].clamp(min=1e-6)
        abar_p = sched.alphas_cumprod[t_prev].clamp(min=1e-6) if t_prev > 0 else torch.ones(1, device=device)
...
# ──────────────────────────────────────────────────────────────
# 10. TRAINING LOOP  [v0.7: + input perturbation]
# ──────────────────────────────────────────────────────────────
...
def train(cfg: Config, device: torch.device) -> dict:
    ...
    log.info(f"Phase: {cfg.phase_name}  [v0.7]")
    log.info(f"Schedule: {cfg.schedule} (shift={cfg.cosine_shift}, zero_snr={cfg.use_zero_snr})")
    log.info(f"Prediction: {cfg.prediction_type}  |  Weighting: {cfg.loss_weighting} (γ={cfg.min_snr_gamma})")
    log.info(f"Input perturbation γ: {cfg.input_pert_gamma}  |  Augmentation: {cfg.augmentation_level}")
    ...
    sched = NoiseScheduler(
        ...
        use_zero_snr=cfg.use_zero_snr,
    ).to(device)

    # Log schedule boundary values to verify zero-SNR behaviour
    log.info(f"Schedule check: ᾱ(0)={sched.alphas_cumprod[0].item():.4f}  "
             f"ᾱ(T/2)={sched.alphas_cumprod[cfg.T//2].item():.4f}  "
             f"ᾱ(T)={sched.alphas_cumprod[-1].item():.6f}")
    ...
    history = {
        ...
        "use_zero_snr": cfg.use_zero_snr,
        "input_pert_gamma": cfg.input_pert_gamma,
    ...
    # Intermediate FID uses DDIM 50 (fast, compatible with v-prediction)
    fid_cfg = copy.deepcopy(cfg)
    fid_cfg.sampler = "ddim"
    ...
    for epoch in range(1, cfg.epochs + 1):
        ...
        for i, x0 in enumerate(tqdm(train_loader,
            ...
            with autocast("cuda", enabled=cfg.use_fp16):
                # [v0.7 NEW] Input perturbation (Ning et al. 2023)
                # Perturb x₀ proportional to sqrt(1-ᾱ_t) so perturbation scale
                # matches the noise level at that timestep. Target remains
                # based on original x₀ — this is precisely the point.
                if cfg.input_pert_gamma > 0:
                    sqrt_one_minus_abar_t = sched.sqrt_one_minus_alphas_cumprod[t][:, None, None, None]
                    pert = cfg.input_pert_gamma * sqrt_one_minus_abar_t * torch.randn_like(x0)
                    x0_pert = x0 + pert
                else:
                    x0_pert = x0

                # Forward process uses perturbed x₀
                xt, noise = sched.q_sample(x0_pert, t)

                # Target uses ORIGINAL x₀ (intentional — that's what we want
                # the model to learn to recover)
    ...
    t0 = time.time()
    ddim_sample(ema.shadow, sched, shape, device, n_steps=50, prediction_type=pt)
    history["sample_time_fast"] = time.time() - t0

    log.info(f"DDPM 1000 steps: {history['sample_time_1000']:.1f}s")
    log.info(f"DDIM 50 steps:   {history['sample_time_fast']:.1f}s")
...
def parse_args():
    p = argparse.ArgumentParser(description="DDPM pipeline v0.7 — precision tuning")
    ...
    p.add_argument("--log_dir", default="./ddpm_runs_v07", help="Output directory")
    ...
    p.add_argument("--zero_snr", choices=["on", "off"], default=None, help="Override zero-SNR rescaling")
    p.add_argument("--input_pert", type=float, default=None, help="Override input perturbation γ")
    p.add_argument("--aug", choices=["weak", "strong"], default=None, help="Override augmentation level")
...
def main():
    ...
    if args.zero_snr   is not None: overrides["use_zero_snr"]        = (args.zero_snr == "on")
    if args.input_pert is not None: overrides["input_pert_gamma"]    = args.input_pert
    if args.aug        is not None: overrides["augmentation_level"]  = args.aug
    ...
    for ph in phases:
        ...
        print(f"\n{'='*70}")
        print(f"  Phase {ph}: {cfg.phase_name}  [v0.7 — precision tuning]")
        print(f"  zero_snr={cfg.use_zero_snr}  input_pert_γ={cfg.input_pert_gamma}  "
              f"aug={cfg.augmentation_level}")
        print(f"  prediction={cfg.prediction_type}  weighting={cfg.loss_weighting}(γ={cfg.min_snr_gamma})")
        print(f"  schedule={cfg.schedule} (shift={cfg.cosine_shift})")
        print(f"  sampler={cfg.sampler}({cfg.sample_steps})  epochs={cfg.epochs}")
        print(f"{'='*70}\n")
        h = train(cfg, device)
