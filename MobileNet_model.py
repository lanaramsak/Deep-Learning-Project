...
MobileNet here is trained on the data. It replaces the head with a custom binary classifier and fine-tunes in two phases:
...
def get_trained_MobileNet_model(train_loader, device):
    ...
    for param in mobilenet.classifier.parameters():
        param.requires_grad = True # Unfreeze the new head layers
